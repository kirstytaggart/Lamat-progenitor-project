...
# Adapted for python3 by Kirsty Taggart 
##############################################################
...
# trilogy2.py - Python Image Library
# pyfits - FITS handler
...
defaultvalues = {
    'indir': '/Users/Dominic/Lamat/SN2021do/trilogy',
    #'outname':'trilogy',
    'outname': 'trilogy',
    'outdir': '/Users/Dominic/Lamat/SN2021do/trilogy',
...
class meanstd_robust:
    ...
    def run(self):
        ...
        for i in range(self.n):
            ...
            #print len(xs)
            imed = (ilo+ihi) / 2
            #print(xs,imed, type(xs),type(imed),len(xs))
            aver = xs[int(imed)]
            #print 'std'
            std1 = std(xs)
            std1 = rms(xs - aver)
...
def loadfitsimagedata(image, indir='', silent=1, bscale=1, bzero=0):
    ...
    #print(shape(hdulist[1].data))
    #print(shape(hdulist[2].data))
    #print(shape(hdulist[3].data))
    ...
    #print 'hey', silent
    if not silent:
        print(image+'[%d]' % iext, shape(data), imfilt)
...
class Trilogy:
    ...
    def loadimagesize(self):
        ...
        for channel in self.mode[::-1]:
            ...
            for iimage, image in enumerate(self.imagesRGB[channel]):
                ...
                #data = loadfitsimagedata(image, self.indir, silent=1)
                data = loadfitsimagedata(image, self.indir)
    ...
    def loadstamps(self, limits, silent=1):
        ...
        three = len(self.mode)
        stampRGB = zeros((three, int(ny), int(nx)), dtype =float)
        print(self.weightext)

        weighting = self.weightext != None

        if weighting:
            weightstampRGB = zeros((three, int(ny), int(nx)), dtype =float)
        ...
        for ichannel, channel in enumerate(self.mode):
            for image in self.imagesRGB[channel]:
                ...
                #print(ylo,yhi)
                #print(type(ylo))
                stamp = data[int(ylo):int(yhi),int(xlo):int(xhi)]
                ...
                #print(ichannel,stamp)
                #print(shape(stampRGB[ichannel] ),shape(stamp))
                #print(stampRGB[ichannel] )
    ...
    def determinescalings(self):
        ...
        while redo:  # Until user is happy with test image of core
            ...
            #print(type(dy/2),type(self.sampledy))
            ylo = clip(self.yc-dy/2 + float(self.sampledy), 0, self.ny)
            yhi = clip(self.yc+dy/2 + float(self.sampledy), 0, self.ny)
            xlo = clip(self.xc-dx/2 + float(self.sampledx), 0, self.nx)
            xhi = clip(self.xc+dx/2 + float(self.sampledx), 0, self.nx)
            ...
            if self.sampledx or self.sampledy:
                print('offset by (%d,%d)' % (float(self.sampledx), float(self.sampledy)))
            ...
            if self.testfirst:
                ...
                outfile = '%s_test_%s_%s_%s.png' % (self.outname, self.satpercent, self.noiselum, self.colorsatfac)
                outfile = join(self.outdir, outfile)
                ...
                line = '  noise yields brightness: %s? ' % self.noiselum
                #inp = input(line)
                inp = input(line)
                #print(inp.strip())
                #print(inp.strip())
                ...
                line = '  %% of pixels that saturate: %s? ' % self.satpercent
                #inp = input(line)
                inp = input(line)
                ...
                if self.mode == 'RGB':
                    line = '  color saturation factor: %s? ' % self.colorsatfac
                    inp = input(line)
                    if inp.strip() != '':
                        self.colorsatfac = float(inp)
                        redo = True

                line = '  Sample size: %s? ' % self.samplesize
                inp = input(line)
                if inp.strip() != '':
                    self.samplesize = int(inp)
                    redo = True

                line = '  Sample offset x: %s? ' % self.sampledx
                inp = input(line)
                if inp.strip() != '':
                    self.sampledx = int(inp)
                    redo = True

                line = '  Sample offset y: %s? ' % self.sampledy
                inp = input(line)
                if inp.strip() != '':
    ...
    def determinescalings2(self):
        ...
        while redo:  # Until user is happy with test image of core
            ...
            if self.testfirst:
                ...
                line = '  noise image input data value: %s? ' % self.noise
                inp = input(line)
                if inp.strip() != '':
                    self.noise = float(inp)
                    redo = True

                line = '  saturation level image input data value: %s? ' % self.saturate
                inp = input(line)
                if inp.strip() != '':
    ...
    def makecolorimage(self):
        ...
        for yo in range(int(self.ylo),int(self.yhi),int(dy)):
            #print('f',self.yhi,yo)
            dy1 = min([int(dy), int(self.yhi)-int(yo)])
            for xo in range(int(self.xlo),int(self.xhi),int(dx)):
                dx1 = min([int(dx), int(self.xhi)-int(xo)])
                print('%5d, %5d  /  (%d x %d)' % (xo, yo, self.nx, self.ny))
                #print dx, dy, self.nx, self.ny, dx1, dy1
                #stamps = self.dataRGB[:,yo:yo+dy,xo:xo+dx]
                #print(yo,dy,xo)
                limits = int(yo), int(yo)+int(dy), int(xo), int(xo)+int(dx)
                ...
                if self.show and self.showstamps:
                    #im.show()
                    #sys.exit()
                    print('this makes plots go into overload if you plot for large img ..')
...
if __name__ == '__main__':    
    if len(sys.argv) == 1:
        infile = '/Users/Dominic/Lamat/SN2021sjt/sjt_trilogy/trilogy_2021sjtmine.in'
        images = None
